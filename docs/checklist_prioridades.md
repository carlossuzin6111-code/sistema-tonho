...
| **BUS-07** | `../backend/src/db/migrations/202607290010_add_session_activity.js`, `workoutSessionController.js`, `index.js`, `../frontend/js/student.js`, `../frontend/js/events.js` | `../backend/src/tests/migrations.test.js`, `workoutSessions.test.js`, `../frontend/tests/student-session.test.js` | `QA visual em dispositivos móveis, retomada após expiração/offline e métricas de uso` |
...
| 29/07/2026 | BUS-07 | Sessão ativa no frontend com início/conclusão/cancelamento, cronômetro de duração, temporizador de descanso, heartbeat único a cada 30s, recuperação via `sessionStorage` e sincronização de exercícios | Branch `bus/bus-07-session-timer`; frontend + backend; `git diff --check` aprovado | Parcial: QA visual em dispositivos móveis, retomada após expiração/offline e métricas de uso |

...
| **OPS-03** | `../backend/src/db/migrations/202607290017_create_partner_consents.js`, `../backend/src/controllers/partnerController.js`, `../backend/src/index.js` | `../backend/src/tests/migrations.test.js`, `../backend/src/tests/api.test.js` | `Onboarding/convites, upload de exames, UI de consentimento e auditoria/telemetria completa` |
...
| 29/07/2026 | OPS-03 | Consentimento explícito do aluno para parceiro clínico, escopos (`workout_logs`, `measurements`, `exams`), expiração, revogação auditável e resumo somente leitura protegido por consentimento ativo | Branch `ops/ops-03-partner-access`; migration `202607290017_create_partner_consents.js`; testes focados 74/74 | Parcial: onboarding/convites, upload de exames, UI de gestão e telemetria completa ainda pendentes |

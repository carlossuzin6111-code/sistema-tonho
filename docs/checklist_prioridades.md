...
| **OPS-12** | `../backend/src/db/migrations/202607290024_create_impersonation_events.js`, `../backend/src/controllers/impersonationController.js`, `../backend/src/middleware/auth.js` | `../backend/src/tests/impersonationController.test.js`, `migrations.test.js` | `Backoffice visual, aprovação dupla, restrições de escopo e telemetria/alerta de uso` |
...
| 29/07/2026 | UX-08 | Quota agregada configurável de 20 MiB para imagens Base64 de exercícios, validação de MIME/assinatura mantida e respostas `413` para excesso; avatar já possui quota e reconciliação por usuário | Branch `ux/ux-08-upload-quotas`; backend focado 82/82 | Parcial: migrar novos uploads para multipart e executar reconciliação global periódica |
| 29/07/2026 | OPS-12 | Impersonação restrita a `support`/`admin`, motivo obrigatório, token Bearer de 15 minutos, evento persistido, listagem/revogação e middleware que bloqueia evento expirado/revogado | Branch `ops/ops-12-impersonation-audit`; migration `202607290024_create_impersonation_events.js`; testes focados 8/8 | Parcial: backoffice visual, aprovação dupla, escopos mínimos e alertas de uso ainda pendentes |

...
| **OPS-07** | `../backend/src/db/migrations/202607290021_create_readiness_checkins.js`, `../backend/src/services/readinessService.js`, `../backend/src/controllers/readinessController.js` | `../backend/src/tests/readinessController.test.js`, `migrations.test.js` | `Bloqueio de abertura da ficha, notificações e calibração clínica das recomendações` |
...
| 29/07/2026 | UX-08 | Quota agregada configurável de 20 MiB para imagens Base64 de exercícios, validação de MIME/assinatura mantida e respostas `413` para excesso; avatar já possui quota e reconciliação por usuário | Branch `ux/ux-08-upload-quotas`; backend focado 82/82 | Parcial: migrar novos uploads para multipart e executar reconciliação global periódica |
| 29/07/2026 | OPS-07 | Check-in diário 1–5 para DOMS, sono, fadiga e humor com upsert por data, score normalizado, recomendação de recuperação/manutenção/normal e leitura protegida pelo Personal vinculado | Branch `ops/ops-07-readiness-checkin`; migration `202607290021_create_readiness_checkins.js`; testes focados 7/7 | Parcial: bloquear abertura da ficha, notificações, histórico clínico e calibração/revisão profissional ainda pendentes |

...
| **BUS-14** | `../backend/src/db/migrations/202607290013_add_catalog_governance.js`, `../backend/src/controllers/exerciseController.js`, `index.js` | `../backend/src/tests/migrations.test.js`, `api.test.js` | `Catálogo global realmente compartilhado, aprovação/moderação e deduplicação automática em lote` |
...
| 29/07/2026 | BUS-14 | Governança inicial adiciona escopo `global/custom`, nome canônico e arquivamento; Personal pode consultar duplicatas e mesclar registros com atualização transacional das fichas e auditoria | Branch `bus/bus-14-catalog-governance`; backend API/migrations 72/72 | Parcial: catálogo global compartilhado, moderação e deduplicação em lote ainda pendentes |

...
| **BUS-09** | Chaves de Cadastro via CLI | 10/10 | **Implementado** | 8h | Baixo | Backend |
...
| 29/07/2026 | BUS-09 | CLI `create/list/revoke <id>` para emitir chaves exibidas uma única vez, listar apenas metadados e revogar chaves não utilizadas | Branch `bus/bus-09-access-key-cli`; accessKeyService 4/4; `git diff --check` aprovado | Concluído no escopo local; gestão de segredos do ambiente permanece operacional |

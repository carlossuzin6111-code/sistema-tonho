...
| 29/07/2026 | BUS-08 | Fila IndexedDB para sessões, reenvio FIFO ao voltar a conexão, `Idempotency-Key` por operação, deduplicação 2xx no backend, retenção máxima de 100 itens/7 dias e API `getOfflineQueueStatus()` para telemetria | Branch `bus/bus-08-offline-retention`; testes frontend + `git diff --check` aprovados | Parcial: ampliar mutações elegíveis e integrar telemetria ao suporte |

...
| **OPS-11** | `../backend/src/db/migrations/202607290023_create_user_sessions.js`, `../backend/src/services/sessionService.js`, `../backend/src/middleware/auth.js`, `../backend/src/controllers/sessionController.js` | `../backend/src/tests/sessionController.test.js`, `migrations.test.js` | `Limpeza/expiração automática, gerenciamento visual e telemetria de dispositivos` |
...
| 29/07/2026 | UX-08 | Quota agregada configurável de 20 MiB para imagens Base64 de exercícios, validação de MIME/assinatura mantida e respostas `413` para excesso; avatar já possui quota e reconciliação por usuário | Branch `ux/ux-08-upload-quotas`; backend focado 82/82 | Parcial: migrar novos uploads para multipart e executar reconciliação global periódica |
| 29/07/2026 | OPS-11 | Sessões persistidas por dispositivo com ID opaco, user-agent/IP, last-seen, listagem e revogação individual; tokens novos validam status ativo sem invalidar outros dispositivos | Branch `ops/ops-11-device-sessions`; migration `202607290023_create_user_sessions.js`; testes focados 6/6 | Parcial: expiração/retenção automática, UI de dispositivos e telemetria operacional ainda pendentes |

...
| **OPS-13** | `../backend/src/services/logger.js`, `metricsService.js`, `../backend/src/index.js` | `../backend/src/tests/logger.test.js`, `metrics.test.js` | `Exportação Prometheus/OpenTelemetry, alertas, retenção e correlação distribuída` |
...
| 29/07/2026 | UX-08 | Quota agregada configurável de 20 MiB para imagens Base64 de exercícios, validação de MIME/assinatura mantida e respostas `413` para excesso; avatar já possui quota e reconciliação por usuário | Branch `ux/ux-08-upload-quotas`; backend focado 82/82 | Parcial: migrar novos uploads para multipart e executar reconciliação global periódica |
| 29/07/2026 | OPS-13 | Logger JSON com request ID, redaction recursivo de credenciais, métricas de requisição em memória e endpoint protegido para suporte/admin | Branch `ops/ops-13-observability`; testes focados 2/2 | Parcial: exportação Prometheus/OpenTelemetry, alertas, retenção e armazenamento distribuído ainda pendentes |

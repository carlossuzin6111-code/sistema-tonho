...
| **OPS-05** | `../backend/src/db/migrations/202607290019_create_crm_alerts_and_nps.js`, `../backend/src/services/crmService.js`, `../backend/src/controllers/crmController.js` | `../backend/src/tests/crmController.test.js`, `migrations.test.js` | `Agendamento node-cron/worker, envio de e-mail/push e métricas de conversão/churn` |
...
| 29/07/2026 | OPS-05 | Núcleo diário de churn/NPS: detecta alunos sem sessão há pelo menos 5 dias, deduplica alerta por dia, cria pesquisa pendente, permite resposta 0–10, resolução e auditoria com ownership | Branch `ops/ops-05-crm-alerts`; migration `202607290019_create_crm_alerts_and_nps.js`; testes focados 8/8 | Parcial: ligar a scheduler persistente, entrega de e-mail/push, dashboards e métricas operacionais ainda pendentes |
